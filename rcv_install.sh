...
echo "getting $script_name..."
wget /usr/local/bin/restart_cctv-viewer.sh https://raw.githubusercontent.com/tylermatthew/cctv-viewer-memleak-fix/main/restart_cctv-viewer.sh; wait
...
ls /usr/local/bin/ | grep restart >/dev/null || echo "ERROR: No rcv in user's bin folder!" && echo "$(date) ERROR: No rcv in user's bin folder!" >> /var/log/$script_name.log
ls /var/log/ | grep restart >/dev/null || echo "ERROR: No $script_name log file!" && echo "$(date) ERROR: No $script_name log file!" >> /var/log/$script_name.log
ls /etc | grep rc.local >/dev/null || echo "ERROR: No rc.local file!" && echo "$(date) ERROR: No rc.local file!" >> /var/log/$script_name.log
cat /etc/rc.local | grep $script_name >/dev/null || echo "ERROR: $script_name not found in the rc.local file!" && echo "$(date) ERROR: $script_name not found in the rc.local file!" >> /var/log/$script_name.log
